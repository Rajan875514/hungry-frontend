...
import { STRIPE_Publishable_key } from "../../utils/clientConfig";
...
export const placeorder = createAsyncThunk(
  ...
  async ([resId, cartItems]) => {
    ...
    try {
      ...
      // Call backend without resId in URL
      const session = await axiosInstance.post("/order/placeorder", {
        items: cartItems,
      });
      ...
      // Optionally return some success info here if needed
      return result;
    } catch (error) {
      toast.error(error?.response?.data?.error || "Something went wrong", { id: loadingMessage });
      throw error;
    }
  }
);
...
const orderSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromPreOrder: (state, action) => {
      ...
      console.log(state.AllPrepredOrders);
    },
